...
const ENCRYPTION_CONFIG = {
    name: 'AES-GCM',
    ivLength: 12,
    saltLength: 32,          // увеличено с 16 до 32
    iterations: 600000,      // увеличено с 100000 до 600000
    hash: 'SHA-256'
};
...
App.db.encryption.initMasterKey = async function(password, salt) {
    // НЕ проверяем длину пароля здесь – это может быть существующий короткий пароль
    let finalSalt = salt;
    if (!finalSalt) {
...
App.db.encryption.changeMasterPassword = async function(oldPassword, newPassword) {
    // Проверяем новый пароль (минимум 8 символов)
